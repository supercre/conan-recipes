...
class LibuvConan(ConanFile):
    ...
    license = "MIT"
    topics = ("libuv", "asynchronous", "io", "networking", "cross-platform")
    ...
    def source(self):
        """소스 코드 다운로드"""
        # Git을 사용하여 libuv 소스 다운로드
        git = Git(self)
        git.clone(url="https://github.com/libuv/libuv.git",
    ...
    def generate(self):
        ...
        tc.variables["LIBUV_BUILD_BENCH"] = False        
    ...
    def package_info(self):
        """패키지 정보 설정"""
        # Windows에서는 정적 라이브러리 이름을 'libuv'로 지정
        ...
        # 플랫폼별 시스템 라이브러리
        if self.settings.os == "Windows":
            self.cpp_info.system_libs = ["ws2_32", "psapi", "iphlpapi", "userenv"]
        ...
        self.cpp_info.set_property("cmake_target_name", "libuv::uv")
